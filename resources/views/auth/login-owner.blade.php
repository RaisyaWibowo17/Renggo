<x-layouts.guest title="Masuk Pemilik UMKM">
    <div class="mb-8 text-center">
        ...
        <p class="mt-1 text-sm text-brand-500">Masuk sebagai Pemilik UMKM</p>
    </div>

    <form action="{{ route('login.owner') }}" method="POST" class="space-y-4">
        @csrf
        <div>
            <label class="field-label">Username</label>
            <input type="text" name="identifier" value="{{ old('identifier') }}" class="input-field" placeholder="username Anda" required autofocus>
        </div>
        <div>
    ...
    <p class="mt-6 text-center text-sm text-brand-500">
        Belum punya akun? <a href="{{ route('register.owner') }}" class="font-semibold text-brand-900">Daftar</a>
    </p>
    <p class="mt-2 text-center text-xs text-brand-400">
        Anda Pelanggan? <a href="{{ route('login.customer') }}" class="font-semibold text-brand-700">Masuk di sini</a>
    </p>
</x-layouts.guest>
